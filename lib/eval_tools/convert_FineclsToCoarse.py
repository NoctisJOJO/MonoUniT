...
def read_kitti_cal(calfile):
    ...
    parsed = np.array(text_file["cam_K"]).reshape(3, 3)
    p2 = np.zeros([4, 4], dtype=float)
    p2[0, 0] = parsed[0, 0]
    p2[0, 1] = parsed[0, 1]
    p2[0, 2] = parsed[0, 2]
...
def convert_fine2coarse(coarse_gt_path="./"):
    ...
    if not os.path.exists(coarse_gt_path):
        print("mkdir", f"coarse_gt_path")
        os.makedirs(coarse_gt_path, exist_ok=True)

    coarse_gt_path = coarse_gt_path + "/label_2_4cls_for_train"
    if not os.path.exists(coarse_gt_path):
        os.makedirs(coarse_gt_path, exist_ok=True)

    for name in tqdm(list_seq):
        name_key = name.strip()
        ...
        list_gt = json.load(open(file_gt))
        ...
        for ind, gt in enumerate(list_gt):
            ...
            type = gt["type"].lower()
            if type in fine2coarse.keys():
                gt_new = copy.deepcopy(gt)
                gt_new["type"] = fine2coarse[type]
                # gt_new = " ".join(gt_new)
                list_gt_new.append(gt_new)

        with open(os.path.join(coarse_gt_path, "%s" % name.strip()), "w") as f:
            # f.writelines(list_gt_new)
            json.dump(list_gt_new, f, indent=2)
...
if __name__ == "__main__":
    ...
    #     "--coarse_label_path",
    #     default="coarse_gt_label",
    #     help="coarse_new_gt_label_path",
    # )
    # args = argparser.parse_args()
    # convert_fine2coarse(coarse_gt_path=args.coarse_label_path)
    convert_fine2coarse("/root/MonoUniT/dataset/V2X-Seq/infrastructure-side")
