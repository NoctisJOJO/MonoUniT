...
def eval_from_scrach(log_, gt_dir, det_dir, eval_cls_list=None, ap_mode=40):
    ...
    for i, f in enumerate(tqdm(all_f)):
        ...
        gt = load_json_annotations(os.path.join(gt_dir, f), False)
        det = load_json_annotations(os.path.join(det_dir, f), True)
        all_gt.append(gt)
        all_det.append(det)
...
def get_empty_template(is_det=False):
    """从json文件获得数据的模板

    Args:
        is_det (bool, optional): gt真值时False; det检测值时True(有score键)
    """
    ...
    empty_template = {
        ...
        "name": np.zeros((0,), dtype="<U50"),  # 假设类型名最长50字符
        "location": np.zeros((0, 3), dtype=np.float32),
        "dimensions": np.zeros((0, 3), dtype=np.float32),
        "rotation_y": np.zeros((0,), dtype=np.float32),
    }
    if is_det:
        empty_template["score"] = np.zeros((0,), dtype=np.float32)
    return empty_template


def load_json_annotations(json_path, is_det=False):
    """从json文件中以指定格式获得数据

    Args:
        json_path: json文件的地址
        is_det : det检测值含有"score"键.
    """
    # 获取空模板
    empty_template = get_empty_template(is_det)

    with open(json_path, "r") as f:
        data = json.load(f)

    if not data:  # 空文件直接返回模板
        return empty_template.copy()

    # 初始化字段
    annotations = {key: [] for key in empty_template}
    ...
    for obj in data:
        ...
        annotations["alpha"].append(obj["alpha"])
        annotations["occluded"].append(obj["occluded_state"])
        annotations["truncated"].append(obj["truncated_state"])
        annotations["name"].append(obj["type"])
        annotations["rotation_y"].append(obj["rotation"])

        # 3D 位置
        annotations["location"].append(
            [
                obj["3d_location"]["x"],
                obj["3d_location"]["y"],
                obj["3d_location"]["z"],
        ...
        # 如果是检测结果，提取 score
        if is_det:
            annotations["score"].append(obj["score"])  # 假设 JSON 中有 "score" 字段

    # 转换为 NumPy 数组
    final_annotations = {}
    for key in annotations:
        final_annotations[key] = np.array(
            annotations[key], dtype=empty_template[key].dtype
        )

    # 强制形状
    final_annotations["bbox"] = final_annotations["bbox"].reshape(-1, 4)
    final_annotations["location"] = final_annotations["location"].reshape(-1, 3)
    final_annotations["dimensions"] = final_annotations["dimensions"].reshape(-1, 3)
