...
def main_worker(local_rank, nprocs, args):
    ...
    if not args.evaluate and local_rank == 0:  # 如果不是评估模式，并且是主进程
        if os.path.exists(
            os.path.join(cfg["trainer"]["log_dir"], "lib/")
        ):  # 检查日志目录
        ...
        shutil.copytree(
            "./lib", os.path.join(cfg["trainer"]["log_dir"], "lib/")
        )  # 将当前lib目录复制到日志目录内
    ...
    if args.seed is not None:
        ...
        warnings.warn(
            ...
            "from checkpoints."
        )  # 警告用户使用随机种子可能会降低训练速度，并可能导致从检查点恢复时出现意外行为
    ...
    dist.init_process_group(
        ...
        rank=local_rank,
    )  # 当前进程的rank（用于标识进程或区分不同的计算资源）

    os.makedirs(cfg["trainer"]["log_dir"], exist_ok=True)  # 创建日志目录
    logger = create_logger(
        os.path.join(cfg["trainer"]["log_dir"], "train.log")
    )  # 创建日志

    # 加载训练数据并创建DataLoader
    train_set = Rope3D(
        root_dir=cfg["dataset"]["root_dir"], split="train", cfg=cfg["dataset"]
    )  # 创建训练集
    ...
    val_set = Rope3D(
        root_dir=cfg["dataset"]["root_dir"], split="val", cfg=cfg["dataset"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ip", default=1222, type=int, help="node rank for distributed training"
    )
