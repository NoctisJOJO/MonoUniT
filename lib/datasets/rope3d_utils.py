...
class Object3d(object):
    def __init__(self, line):
        ...
        self.occlusion = float(label[2])  # 障碍物遮挡 0:fully visible 1:partly occluded 2:largely occluded 3:unknown
    ...
    def get_obj_level(self):  # 物体检测的难度级别
        height = float(self.box2d[3]) - float(self.box2d[1]) + 1  # ymax - ymin + 1 (+1确保高度为正值)
    ...
    def to_bev_box2d(self, oblique=True, voxel_size=0.1):
        ...
        if oblique:
            ...
            xz_corners = corners3d[0:4, [0, 2]]  # 取前4个角点的X和Z坐标（忽略Y轴）（XZ平面）
            ...
            box2d[:, 0] = np.clip(box2d[:, 0], 0, Object3d.BEV_SHAPE[1])  # 将边界框的X和Z坐标限制在鸟瞰图的范围内，
            box2d[:, 1] = np.clip(box2d[:, 1], 0, Object3d.BEV_SHAPE[0])  # 防止坐标超出图像边界
        else:
...
class Denorm(object):
    ...
    def __init__(self, denorm_file):
        ...
        for line in text_file:
            parsed = line.split('\n')[0].split(' ')  # 0.008187128 -0.975265 -0.2208872 7.23388195038
        ...
        ground_x_axis = ground_x_axis / np.linalg.norm(ground_x_axis)  # 归一化，变成单位向量 结果：与地面法向量ground_z垂直的地面X轴
        ground_y_axis = np.cross(ground_z_axis, ground_x_axis)  # 外积\叉乘  得到与两个向量垂直的向量
    ...
    def ground_center2object_center(self,camera_pos,h): # camera_pos是物体中心在相机坐标系下的坐标
        ground_pos = np.matmul(self.c2g_trans,camera_pos)  # 将相机坐标系转换为地面坐标系
        ground_pos[2,0] += h/2  # 修正地面坐标中的Z轴，考虑物体高度的一半
        camera_center = np.matmul(self.g2c_trans,ground_pos)  # 将修正后的地面坐标系转换回相机坐标系
...
def get_calib_from_file(calib_file):
    ...
    obj = lines[0].strip().split(' ')[1:]  # strip()去除首尾的空格，split以空格为分界，[1:]跳过第一个
...
class Calibration(object):
    ...
    def depthmap_to_rect(self, depth_map):
        ...
        depth = depth_map[y_idxs, x_idxs]  # 根据x_idxs, y_idxs从深度图中提取出每个像素的深度值，存入一维数组depth
    ...
    def corners3d_to_img_boxes(self, corners3d):
        ...
        corners3d_hom = np.concatenate((corners3d, np.ones((sample_num, 8, 1))), axis=2)  # (N,8,3)+(N,8,1)=(N, 8, 4)

        img_pts = np.matmul(corners3d_hom, self.P2.T)  # (N, 8, 3)  # 投影到二维图像平面

        x, y = img_pts[:, :, 0] / img_pts[:, :, 2], img_pts[:, :, 1] / img_pts[:, :, 2]  # /Z 见上面的投影函数; x和y为(N,8)
        x1, y1 = np.min(x, axis=1), np.min(y, axis=1)  # 最大值和最小值
        x2, y2 = np.max(x, axis=1), np.max(y, axis=1)  # 也可以看成目标框左上角(x1,y1)和右下角(x2,y2)

        boxes = np.concatenate((x1.reshape(-1, 1), y1.reshape(-1, 1), x2.reshape(-1, 1), y2.reshape(-1, 1)), axis=1)
        boxes_corner = np.concatenate((x.reshape(-1, 8, 1), y.reshape(-1, 8, 1)), axis=2)  # (N,8,2) 得到每个角点在图像平面中的坐标
    ...
    def inverse_rigid_trans(self, Tr):
        ...
        inv_Tr[0:3, 0:3] = np.transpose(Tr[0:3, 0:3])  # 旋转矩阵是正交矩阵，正交矩阵的转置矩阵就是逆矩阵
    ...
    def affine_transform(self,img_size,trans):
        ...
        wsize = 4
        hsize = 2
        random_depth = np.linspace(2,78,wsize*hsize).reshape(hsize,wsize,1)  # 深度从2~78均匀分布，分成w*h份
        ...
        p2ds[:,:2] = np.dot(np.concatenate([p2ds[:,:2],np.ones([wsize*hsize,1])],-1),trans.T)  # 仿射变换
...
def get_3rd_point(a, b):
    ...
    return b + np.array([-direct[1], direct[0]], dtype=np.float32)  # 后面的意思是将direct逆时针旋转九十度
                                                                           # 最后得到等腰直角三角形的斜边
...
def get_affine_transform(center,
    ...
    if not isinstance(scale, np.ndarray) and not isinstance(scale, list):
        scale = np.array([scale, scale], dtype=np.float32)  # 将scale转换为NumPy数组格式，确保是长度为2的数组[sx,sy]，即缩放比例
    ...
    src = np.zeros((3, 2), dtype=np.float32)  # 存储源图像和目标图像的三个关键点坐标，用于得出仿射变换矩阵
...
def compute_box_3d(obj, calib):
    ...
    corners_3d[0,:] = corners_3d[0,:] + obj.pos[0];  #绝对位置
...
if __name__ == '__main__':
    ...
    for object in objects:
        ...
        object.pos[0] *= 1
        center_3d = object.pos + [0, -object.h/2, 0]   # real 3D center object是底部位置，向上半个高度即中心
